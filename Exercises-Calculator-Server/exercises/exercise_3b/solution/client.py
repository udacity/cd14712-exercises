...
async def run():
    async with streamablehttp_client(url="http://localhost:8000/mcp") as (read, write, _):
        async with ClientSession(read, write) as session:
            ...
            if prompts.prompts:
                prompt = await session.get_prompt("solve_equation", arguments={"equation": "2 * 10^10"})
            ...
            resource_content = await session.read_resource(AnyUrl("constants://golden_ratio"))
            ...
            # 7. call tool
            result = await session.call_tool("square_root", arguments={"n": "16"})
